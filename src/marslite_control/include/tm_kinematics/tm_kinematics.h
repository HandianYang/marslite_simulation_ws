/**
 * @file tm_kinematics.h
 * @author Handian Yang
 * @copyright Released under the terms of the GPLv3.0 or later
 * @date 2024
 * 
 * @brief The header file for the kinematics of TM5 robotic arm.
 *      Specifically, the program provides the forward and inverse
 *      kinematics of the TM5-700, TM5-900, TM5-1100, and TM5-1300
 *      robotic arms.
 * @note `tm_kinematics.h` is part of `marslite_control`.
 * 
...
public:
    ...
    explicit TMKinematics(const RobotType& robotType = TM5_700);
...
private:
     ...
     */
    inline const bool distanceCloseToTarget(const double& value,
                                            const double& target = 0) const {
    ...
    inline TransformationMatrix A2(const double& theta) const {
        return getTransformationMatrix(theta, 0, a2_, 0, -M_PI/2);
    }
    ...
    inline TransformationMatrix A4(const double& theta) const {
        return getTransformationMatrix(theta, M_PI_2, 0, d4_, M_PI/2);
    }
